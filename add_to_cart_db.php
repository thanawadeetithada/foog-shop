...
$subtotal = ($price + $extra_cost) * $quantity;

// ตรวจสอบว่ามี order ค้างอยู่ไหม
...
if ($existing_order) {
    ...
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ddi", $subtotal, $extra_cost, $cart_order_id);
    $stmt->execute();
    $stmt->close();
} else {
    $sql = "INSERT INTO cart_orders (user_id, store_id, total_price, extra_cost, options, payment_method, status) 
            VALUES (?, ?, ?, ?, ?, ?, 'Pending')";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("iidsss", $user_id, $store_id, $subtotal, $extra_cost, $options, $payment_method);
    $stmt->execute();
